...
//check for failure data file, if it does not exist, alert message
if (!file_exists("../failure.CC.data.csv")) {
	...
	echo "<script type='text/javascript'>(confirm('$message') == true);</script>";
	exit;
}
...
// open the tokens file and undo.import file
$tokens = fopen("../tokens.CC.csv", "a+");
$undo_import = fopen("../undo.import.CC.csv", "a+");

//open the failure data file and initiate the import loop
$handle = fopen("../failure.CC.data.csv", "r");
while ($fields = fgetcsv($handle, 10000, ",")) {
	...
	//if the failure log file does not exist, create it and write the header
	if (!file_exists('../failure.log2.CC.txt')) {
		$fail_log = fopen("../failure.log2.CC.txt", "a+");
		fwrite($fail_log, "
////////////////////////////////////////////////////////////////
////////////////////                       /////////////////////
////////////////////   C C   E R R O R S   /////////////////////
...
");
	...
	$fail_data = fopen("../failure.data2.CC.csv", "a+");
...
if($fail == 0) {
	...
	unlink("../failure.data2.CC.csv");
	unlink("../failure.log2.CC.txt");
	...
	echo "<script type='text/javascript'>alert('$message');</script>";
		...
		unlink("../failure.CC.data.csv");
		unlink("../failure.CC.log.txt");
		rename("../failure.data2.CC.csv","../failure.CC.data.csv");
		rename("../failure.log2.CC.txt","../failure.CC.log.txt");
		$message = "TOOLBOX 1 says:\\n\\n$success CC customers imported successfully.\\n$fail failures.\\n\\nOpen failure.CC.data.csv and failure.CC.log.txt in Notepad++ to fix the errors.";
		//echo "<script type='text/javascript'>alert('$message'); window.history.back();</script>";
		echo "<script type='text/javascript'>alert('$message');</script>";
	}
?>