...
//This script builds the xml file. First it creates a csv with the select fields we desire, then converts it to XML

date_default_timezone_set('America/Chicago');

//define the header row
$headers = 'MID,Name_on_Card,Card,Last_4,Mo,Year,Updated,AU_Code,AU_Description,Paymethod_Token,Customer_Token,Report_Created_On' . PHP_EOL;
...
while($c >= 0){	
	...
	for($i=0; $i < sizeof($data["results"]); $i++) {
		...
		//define the row
		$entries = $merchant_id.','.$card_name.','.$cc_type.','.$masked_CC.','.$expire_mo.','.$expire_yr.','.$date.','.$code.','.$description.','.$pay_token.','.$cust_token.','.$created_on . PHP_EOL;
