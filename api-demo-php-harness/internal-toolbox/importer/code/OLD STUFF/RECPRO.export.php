...
$headers = '"PaymentID","FamilyID","PaymentToken","ClientToken","FirstName","LastName"'.PHP_EOL;
...
while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
	...
	for ($c=0; $c < $num; $c++) {
		...
		$payment_id      = $data["paymethod"]["notes"];
		$created         = $data["created_date"];
		$created_date    = date('Y-m-d g:i A', strtotime("+2 hours $created"));
		
		// define the row
		$entries = $payment_id.','.$customer_id.','.$paymethod_token.','.$customer_token.','.$firstname.','.$lastname.PHP_EOL;
