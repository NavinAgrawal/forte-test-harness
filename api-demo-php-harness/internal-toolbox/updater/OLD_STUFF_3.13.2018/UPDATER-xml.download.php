...
while($i >= 0){
	...
	//humble attempt at error-handling
	...
	//Loop through the data plucking only the fields we want in our file
	$doc = new DOMDocument();
	...
	foreach( $au_records as $au_record ) 
	{
		...
		//write the data we plucked as a csv file
...
$inputFilename    = 'files/AU.Report.csv';
$newfile   = 'files/AU.Report.xml';
...
header('Content-Disposition: attachment; filename="AU.Report.xml"');
header("Content-Length: " . filesize("files/AU.Report.xml"));

$fp = fopen("files/AU.Report.xml", "r");
fpassthru($fp);

$filename = 'AU.Report.xml';
?>