...
//this script exports all of the customers with "echeck" as default_paymethod_type
...
$filename    = 'EXPORT.ACH.CUSTOMERS--MID.'.$merchant_id.'--'.date("Y.m.d").'.csv';
...
$headers = '"Customer Token","Paymethod Token","Consumer ID","Status","First Name","Last Name","Company Name","Address 1","Address 2",City,State,Zipcode,Phone,Email,"Account Holder","Account Type","Routing No","Last 4","Created On"'.PHP_EOL;
...
while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
	...
	for ($c=0; $c < $num; $c++) {
		...
		// define the row
		$entries = $customer_token.','.$paymethod_token.','.$customer_id.','.$status.','.$firstname.','.$lastname.','.$company.','.$address1.','.$address2.','.$city.','.$state.','.$zipcode.','.$phone.','.$email.','.$accountholder.','.$account_type.','.$routing.','.$ach_last_4.','.$created_date.PHP_EOL;
