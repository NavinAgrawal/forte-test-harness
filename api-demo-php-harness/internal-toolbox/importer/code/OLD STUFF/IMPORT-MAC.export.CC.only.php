...
$filename    = 'MAC_CC_export-'.date("Y.m.d").'.csv';
...
$headers = '"Subscriber ID","New Credit Card Token ","Credit Card Expiration Date","Credit Card Type","Credit Card Bill To Name","Last of Digits of Credit Card Number"'.PHP_EOL;
...
while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
	...
	for ($c=0; $c < $num; $c++) {
		...
		//$expiry          = ($data["paymethod"]["card"]["expire_month"]).'/'.($data["paymethod"]["card"]["expire_year"]);
...
/*		$merchant_id     = str_replace("loc_","",($data["location_id"]));
		...
		$expiry = $expire_mo.$expire_yr;
		
		// define the row
		$entries = $customer_id.','.$paymethod_token.','.$expiry.','.$card_type.','.$cardholder.','.$cc_last_4.PHP_EOL;
