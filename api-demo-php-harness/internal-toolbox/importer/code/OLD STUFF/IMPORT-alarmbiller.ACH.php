...
$filename    = 'AlarmBiller.ACH--MID.'.$merchant_id.'--'.date("Y.m.d").'.csv';
...
$headers = '"CUSTOMER NUMBER","PAYMENT TOKEN","CUSTOMER TOKEN","ROUTING NUMBER","LAST 4","ACCT NAME CUSTOMER NAME","BILLING ADDRESS","CITY","STATE","ZIP","PHONE","EMAIL"'.PHP_EOL;
...
while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
	...
	for ($c=0; $c < $num; $c++) {
		...
		// define the row
		$entries = $customer_id.','.$paymethod_token.','.$customer_token.','.$routing.','.$ach_last_4.','.$accountholder.','.$address1.','.$city.','.$state.','.$zipcode.','.$phone.','.$email.PHP_EOL;
