...
$tokens = fopen("../tokens.ACH.csv", "a+");
$undo_import = fopen("../undo.import.ACH.csv", "a+");

//open the data file and initiate the import loop
$handle = fopen("../failure.ACH.data.csv", "r");
while ($fields = fgetcsv($handle, 10000, ",")) {
	...
	if (!file_exists('../failure.log2.ACH.txt')) {
		$fail_log = fopen("../failure.log2.ACH.txt", "a+");
		fwrite($fail_log, "
////////////////////////////////////////////////////////////////
///////////////////                         ////////////////////
///////////////////   A C H   E R R O R S   ////////////////////
...
");
	...
	$fail_data = fopen("../failure.data2.ACH.csv", "a+");
	...
	if($info['http_code']==201) {
		$success++;
		$count++;
		$fields[14] = substr($fields[14],-4);  //convert card number to last 4 only	
...
if($fail == 0) {
	...
	unlink("../failure.data2.ACH.csv");
	unlink("../failure.log2.ACH.txt");
	...
	echo "<script type='text/javascript'>alert('$message');</script>";
		...
		unlink("../failure.ACH.data.csv");
		unlink("../failure.ACH.log.txt");
		rename("../failure.data2.ACH.csv","../failure.ACH.data.csv");
		rename("../failure.log2.ACH.txt","../failure.ACH.log.txt");
		$message = "TOOLBOX 1 says:\\n\\n$success ACH customers imported successfully.\\n$fail failures.\\n\\nOpen failure.ACH.data.csv and failure.ACH.log.txt in Notepad++ to fix the errors.";
		//echo "<script type='text/javascript'>alert('$message'); window.history.back();</script>";
		echo "<script type='text/javascript'>alert('$message');</script>";
	}
?>