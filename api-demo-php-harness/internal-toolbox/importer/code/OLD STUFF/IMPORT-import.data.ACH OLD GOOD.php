...
array_map("unlink", glob("../../../internal-toolbox/importer/failure.ACH.data.csv"));
array_map("unlink", glob("../../../internal-toolbox/importer/failure.ACH.log.txt"));
...
$headers = '"Customer Token","Paymethod Token","First Name","Last Name","Company Name","Address 1","Address 2","City","State","Zipcode","Phone","Email","Customer ID","Account Holder","Routing No","Last 4","Account Type"'.PHP_EOL;
...
//$file = new SplFileObject('../../../internal-toolbox/importer/data.csv', 'r');
...
while ($fields = fgetcsv($handle, 10000, ",")) {
	...
	$firstname      = str_replace($badcharacters,"",$fields[0]);
	$lastname       = str_replace($badcharacters,"",$fields[1]);
	$companyname    = str_replace($badcharacters,"",$fields[2]);
	$address1       = str_replace($badcharacters,"",$fields[3]);
	$address2       = str_replace($badcharacters,"",$fields[4]);
	$city           = str_replace($badcharacters,"",$fields[5]);
	$state          = $fields[6];
	$zipcode        = $fields[7];
	$phone          = $fields[8];
	$email          = $fields[9];
	$customer_id    = $fields[10];
	$account_holder = str_replace($badcharacters,"",$fields[11]);
	...
	//echo $count + $fail . ' record count<br><br>';
	//print_r($data);
	...
	if (!file_exists('../../../internal-toolbox/importer/failure.ACH.log.txt')) {
		$fail_log = fopen("../../../internal-toolbox/importer/failure.ACH.log.txt", "a+");
		fwrite($fail_log,"
////////////////////////////////////////////////////////////////
//////////////////                         /////////////////////
//////////////////   A C H   E R R O R S   /////////////////////
...
");
	...
	$fail_data = fopen("../../../internal-toolbox/importer/failure.ACH.data.csv", "a+");
	...
	if($info['http_code']==201) {
		$success++;
		$count++;
		$fields[13] = substr($fields[13],-4);  //convert card number to last 4 only	
