...
$filename    = 'AlarmBiller.CC--MID.'.$merchant_id.'--'.date("Y.m.d").'.csv';
$endpoint    = $base_url.'/organizations/'.$organization_id.'/locations/'.$location_id.'/customers/?filter=default_paymethod_type+eq+card&page_size=1000&orderby=customer_token';
$endpoint2   = $base_url.'/organizations/'.$organization_id.'/locations/'.$location_id.'/customers/';
$token_set   = "customer.tokens.csv";

// define the header row
$headers = '"CUSTOMER NUMBER","PAYMENT TOKEN","CUSTOMER TOKEN","CREDIT CARD TYPE","LAST 4","EXP MONTH","EXP YR","ACCT NAME CUSTOMER NAME","BILLING ADDRESS","CITY","STATE","ZIP","PHONE","EMAIL"'.PHP_EOL;
...
while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
	...
	for ($c=0; $c < $num; $c++) {
		...
		$combine         = $expire_yr.$expire_mo;
		
		// define the row
		$entries = $customer_id.','.$paymethod_token.','.$customer_token.','.$card_type.','.$cc_last_4.','.$expire_mo.','.$expire_yr.','.$cardholder.','.$address1.','.$city.','.$state.','.$zipcode.','.$phone.','.$email.PHP_EOL;
