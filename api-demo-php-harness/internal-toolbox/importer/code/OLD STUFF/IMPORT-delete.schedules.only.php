...
//this script deletes all of the existing schedules
...
while($c >= 0){
	...
	if ($schedules_number_results == 0) {
		$message = "There are no schedules to delete.";
		echo "<script type='text/javascript'>if(confirm('$message') == true) {window.history.back();} else {window.history.back();}</script>";
		exit;
	}
...
if($schedulesDeleteCount > 0) {
	$message = "$schedulesDeleteCount schedules deleted successfully.";
	echo "<script type='text/javascript'>if(confirm('$message') == true) {window.history.back();} else {window.history.back();}</script>";
}
